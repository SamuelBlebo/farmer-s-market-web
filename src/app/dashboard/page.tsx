...
import { setProductStatus } from '@/server/actions/products';
...
export default async function DashboardPage() {
  ...
  const waiting = products.filter((p) => p.moderation === 'PENDING').length;
    ...
    <>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="card p-4">
          ...
          <p className="text-[13px] text-muted">Live listings</p>
        </div>
        <div className="card p-4">
          <div className="font-num text-2xl font-bold">{waiting}</div>
          <p className="text-[13px] text-muted">Waiting for approval</p>
        </div>
        <div className="card p-4">
          <div className="font-num text-2xl font-bold">{products.length}</div>
          <p className="text-[13px] text-muted">Listings all time</p>
        </div>
        <div className="card p-4">
          <VerifiedBadge status={profile.verification} />
          <p className="mt-1 text-[13px] text-muted">Farmer status</p>
        </div>
      </div>
        ...
        <div className="card divide-y divide-line">
          {products.map((p) => (
            <div key={p.id} className="flex flex-wrap items-center gap-3 p-3.5">
              <span className="text-2xl" aria-hidden>{p.category.emoji}</span>
