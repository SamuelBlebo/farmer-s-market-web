...
import { InboxSupportRow } from '@/components/inbox-support-row';
...
import { getOrCreateSupportConversation, getSupportConversationPreview } from '@/server/support';

export const metadata: Metadata = { title: 'Messages' };

export default async function MessagesPage({ searchParams }: { searchParams: { id?: string; support?: string } }) {
  const user = await requireUser();
  const isSupportActive = searchParams.support === '1';

  const [conversations, supportPreview] = await Promise.all([
    getConversations(user.id),
    getSupportConversationPreview(user.id),
  ]);

  // Support only ever gets created once the user actually clicks into it —
  // not just from a preview peek on every inbox load.
  const supportConversation = isSupportActive ? await getOrCreateSupportConversation(user.id) : null;

  // Nothing picked yet — the most recent conversation opens by default, same as any inbox.
  const activeId = isSupportActive ? undefined : searchParams.id ?? conversations[0]?.id;
  const active = !isSupportActive && activeId ? await getConversation(activeId, user.id) : null;
    ...
    <div className="mx-auto max-w-[1040px]">
      ...
      <div className="sm:grid sm:grid-cols-[260px_1fr] sm:items-start sm:gap-4 lg:grid-cols-[340px_1fr]">
        <div className="card divide-y divide-line overflow-hidden rounded-2xl shadow-sm">
          {/* Pinned — always first, whether or not a support thread exists yet. */}
          <div className="sm:hidden">
            <InboxSupportRow preview={supportPreview} />
          </div>
          <Link
            href="/messages?support=1"
            className={`hidden items-center gap-3 px-4 py-3 transition-colors sm:flex ${
              isSupportActive ? 'bg-paper' : 'hover:bg-paper'
            }`}
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <span className={`truncate text-[15px] ${supportPreview && supportPreview.unreadCount > 0 ? 'font-bold' : 'font-semibold'}`}>
                  Support
                </span>
                {supportPreview && <span className="shrink-0 text-[12px] text-muted">{chatTimeLabel(supportPreview.lastMessageAt)}</span>}
              </div>
              <div className="mt-0.5 flex items-center justify-between gap-2">
                <p className={`truncate text-[13px] ${supportPreview && supportPreview.unreadCount > 0 ? 'font-semibold text-ink' : 'text-muted'}`}>
                  {supportPreview?.lastMessage ?? 'Chat with our team'}
                </p>
                {supportPreview && supportPreview.unreadCount > 0 && (
          ...
          {conversations.length === 0 ? (
            <p className="p-4 text-[13px] text-muted">
              Start a chat from a listing or a farmer&apos;s page and it&apos;ll show up here too.
            </p>
          ) : (
            conversations.map((c) => (
              <div key={c.id}>
        ...
        <div className="hidden sm:block">
          {isSupportActive && supportConversation ? (
            <div className="card flex h-[calc(100vh-220px)] min-h-[420px] flex-col p-4">
              <div className="mb-3 flex items-center gap-3 border-b border-line pb-3">
                <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-leaf-dark text-white">
                  <ChatIcon className="h-5 w-5" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[16px] font-bold">Support</p>
                  <p className="truncate text-[12.5px] text-muted">An admin usually replies within a day.</p>
                </div>
              </div>
              <div className="min-h-0 flex-1">
                <SupportThread
                  key={supportConversation.id}
                  conversationId={supportConversation.id}
                  viewerIsAdmin={false}
                  initialMessages={supportConversation.messages.map((m) => ({
                    id: m.id,
                    senderId: m.senderId,
            ...
            <div className="card grid h-[calc(100vh-220px)] min-h-[420px] place-items-center p-4 text-center text-sm text-muted">
              <div>
                <MessageIcon className="mx-auto h-6 w-6 text-muted" />
                <p className="mt-2">Select a conversation to view it here.</p>
              </div>
            </div>
          )}
        </div>
